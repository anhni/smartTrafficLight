...
void OpenOutput7(int index);
void CloseOutput7(int index);
...
unsigned char subManualMode = INIT_SYSTEM;
unsigned char settingMode = INIT_SYSTEM;
unsigned char subSettingMode = INIT_SYSTEM;
...
void OpenOutput7(int index)
{
	if (index >= 0 && index <= 7)
	{
		TRISC_OUT = TRISC_OUT | arrayMapOfOutput[index];
	}
}
void CloseOutput7(int index)
{
...
void UartDataReceiveProcess()
{
    ...
    {   
        LcdClearS();
                ...
                LcdPrintNumS(0,4,dataReceive[0]*10 + dataReceive[1]);
                LcdPrintStringS(0,8,"Y1:");
                LcdPrintNumS(0,11,dataReceive[2]*10 + dataReceive[3]);
                LcdPrintStringS(1,0,"G2:");
                LcdPrintNumS(1,4,dataReceive[4]*10 + dataReceive[5]);
                LcdPrintStringS(1,8,"Y2:");
                LcdPrintNumS(1,11,dataReceive[6]*10 + dataReceive[7]);
        ...
        if(enable == 0 && trafficMode == 10){
            UartSendString("\r\nChanged to traffic mode!\r\n");
            enable = 1;
        }
    if(isButton4Up() && trafficMode == MODE_UART){
        trafficMode = MODE_TRAFFIC_LIGHT;
        statusReceive = 1;
    }
}
...
void setting_time(){
    ...
    switch(settingMode){
        case MODE_1:
            ...
            if(waiting_time == 500){
                subSettingMode = settingMode;
                settingMode = LONG_WAITING_MODE;
                waiting_time = 0;
                //settingMode = MODE_1;
            }
            break;
        ...
        case WAITING_NEXT_MODE:  
            ...
            break;
        case LONG_WAITING_MODE:
            LcdClearS();
            LcdPrintStringS(0,0,"YOU STILL HERE ?");  
            LcdPrintStringS(1,0,"YES <");
            if(isButton6Up() || isButton4Up() || isButton5Up()){
                settingMode = subSettingMode;              
                timeDelay = 0;
                enable = 0;
                count = 0;
                waiting_time = 0;
...
void Manual_Adjustment(){
    waiting_time++;
    if(waiting_time == 800 && manualMode != LONG_WAITING_MODE){       
        subManualMode = manualMode;
        manualMode = LONG_WAITING_MODE;
        waiting_time = 0;
    }
    switch(manualMode){
        case INIT_SYSTEM:
            ...
            LcdPrintStringS(1,0," CHANGE   EXIT ");
            if (count == 0){
                LcdPrintStringS(1,0," CHANGE<  EXIT ");
            }else if(count == 1){
                LcdPrintStringS(1,0," CHANGE   EXIT<");
            ...
            if(isButton6Up()){
                ...
                else if(count == 1) manualMode = WAITING_NEXT_MODE;           
                timeDelay = 0;
                enable = 0;
                waiting_time = 0;
            }
        ...
        case MODE_MANUAL_0:
            LcdPrintStringS(0,0," MANUAL MODE:ON ");  
            timeDelay = (timeDelay+1)%100;
            if(timeDelay == 99) enable = 1;
            if(enable == 0){
                waiting_time = 0;
                LcdPrintStringS(1,0,"CHANGE LED IN:   ");
                LcdPrintNumS(1,14,(100 - timeDelay)/20);
                LcdPrintStringS(1,15,"s");
                Phase1_YellowOn();
                Phase1_GreenOff();
                Phase1_RedOff();
...
//                    }
        ...
        case WAITING_NEXT_MODE:
            ...
            if(nextModeFlag == 1){
                ...
                LcdClearS();
                LcdPrintStringS(0,0,"BACK TO TRAFFIC");
                LcdPrintStringS(1,0,"MODE IN:");
                LcdPrintNumS(1,9,waitingTimeTrafficLight);
                LcdPrintStringS(1,11,"S");
            }           
        break;
        case LONG_WAITING_MODE:
            LcdClearS();
